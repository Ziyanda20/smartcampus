...
function MyBookings() {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const url = user.role === 'lecturer' 
        ? `http://localhost:5000/lecturers/${user.id}/bookings`
        : 'http://localhost:5000/booking/my-bookings'; // fallback for students
  ...
  const handleCancel = async (bookingId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const url = user.role === 'lecturer' 
        ? `http://localhost:5000/lecturers/${user.id}/bookings/${bookingId}`
